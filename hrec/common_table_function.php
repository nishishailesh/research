	...
	echo '<div class="container bg-light">';
			echo '<div class="row bg-warning">';	
				...
				echo '<div class="col-sm-4 text-right">';	
					echo '<img src=search.png style="width:5%;">';
				echo '</div>';
				echo '<div class="col-sm-4 text-center">';	
					echo $d.'->'.$t.'<span class="badge badge-danger">Search</span>';
				echo '</div>';
...
function show($link,$d,$t,$data)
{
	...
	//my_print_r($data);
		...
		echo '<input type=hidden name=^database readonly size=\''.strlen($d).'\' value=\''.$d.'\'>';
		echo '<input type=hidden name=^table readonly size=\''.strlen($t).'\' value=\''.$t.'\'>';
	echo '<div class="container bg-warning border '.$pk_str_full.'" style="display:none">';
	...
	echo '</form>';
		...
		//echo '<div class="row">';		
			...
			//if(isset($parent['pk']))
			//{
				//show_parent_button($link,$d,$t,$parent['pk']);
			//}
				//show_dependent_button($link,$d,$t,$pk_filled);		
			//echo '</div>';	
		//echo '</div>';	
